...
export class MetricsController {
  ...
  getAdminMetrics = async (req: Request, res: Response): Promise<void> => {
    try {
      const { rows: revenueRows } = await this.db.query("SELECT SUM(total_price) as total_revenue, COUNT(id) as total_orders FROM orders WHERE status = 'CONFIRMED'");
      const { rows: userRows } = await this.db.query("SELECT COUNT(id) as total_users FROM users");
      const { rows: activeStreams } = await this.db.query("SELECT SUM(viewers) as total_viewers FROM livestreams WHERE status = 'LIVE'");
  ...
  getStreamerMetrics = async (req: Request, res: Response): Promise<void> => {
    try {
      ...
      const { rows: revenueRows } = await this.db.query("SELECT SUM(total_price) as total_revenue, COUNT(id) as total_orders FROM orders WHERE status = 'CONFIRMED' AND shop_id = $1", [user.shopId]);
      const { rows: activeStreams } = await this.db.query("SELECT viewers FROM livestreams WHERE status = 'LIVE' AND shop_id = $1 ORDER BY created_at DESC LIMIT 1", [user.shopId]);
  ...
  getUsers = async (req: Request, res: Response): Promise<void> => {
    try {
      const { rows } = await this.db.query("SELECT id, username, email, role FROM users ORDER BY created_at DESC LIMIT 50");
      res.json(rows.map(r => ({
        id: r.id,
        user: r.username,
        email: r.email,
